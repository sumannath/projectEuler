...
public class Problem14 {
    public static void main(String[] args) {
        ...
        int T = scan.nextInt();
        HashMap<Long, Integer> collatzSteps = new HashMap<>(600,0.5f);
        collatzSteps.put(1L, 0);
        int max, steps;
        long maxN = 0;
        for(int i = 1 ; i <= T ; i++) {
            int N = scan.nextInt();
    ...
    private static int getCollatzSteps(long x, HashMap<Long, Integer> collatzSteps) {
        int steps = 0;
        Integer nSteps = collatzSteps.get(x);
        if(nSteps != null) {
            return nSteps;
        } else {
            if(x % 2 == 0) {
